...
@horario_route.post("/", response_model=HorarioEntrada, status_code=status.HTTP_201_CREATED)
def crear_horario(horario: HorarioEntrada, db: Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([1, 3]))):
